...
These are the **deeper measurement harnesses** behind the benchmark claims. They are not part of the
shipped npm package (dev-only), and they drive the **same engine functions the product uses** — not a
separate demo path.

The one-command, fully-deterministic proof of the headline guarantees is the shipped command itself:

```bash
ihow-memory benchmark        # three-color verdicts and tier-aware governance checks
```
...
| --- | --- | --- | --- |
| `autopromote-precision.mjs` | `node bench/autopromote-precision.mjs` | the automatic promotion gate distinguishes blocked secret/conflict inputs from durable verified, unverified, and flagged tiers; also reports the precision ceiling (provenanced ≠ true) and tier distribution on a fixed mix of facts | **yes** — exit non-zero if a labeled governance outcome is misclassified |
| `recall-quality.mjs` | `node bench/recall-quality.mjs` | recall's deterministic safety guarantees: off-topic prompts inject nothing; a stale/superseded entry is never injected; also reports injection rates with automatic default recall disabled versus the current default policy | **safety part: yes** (gates the exit). This does not measure usefulness with an LLM judge or establish production accuracy. |
...
skeptic reproduce the deeper numbers, not just the headline benchmark. Persistence tier accuracy and
deterministic recall safety are separate measurements; neither proves that the stored prose is true.
