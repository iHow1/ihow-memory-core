...
test('audit reads reconstruct only strict whitelisted events', async (t) => {
  ...
  const malicious = [
    { schemaVersion: 1, id: '00000000-0000-4000-8000-000000000001', at, type: 'checkpoint.rejected', operation: 'artifact.read', reasonCode: 'checkpoint_bad', raw: ['api_', 'key', ': ', 'ABCDEF', '0123456789'].join('') },
...
test('PII is redacted across claims, evidence, and engine-provided anchors with metadata recorded', async (t) => {
  ...
  const result = await createAndFinalize(
    ...
    explicit,
    anchorProvider({ files: [{ path: ['/Users', '/carol@example.org/project/file.ts'].join(''), sha256: ZERO_HASH }], commands: [] }),
  );
