...
test('redactor quarantines prose secrets, URL creds, and keyword-led blobs', () => {
  ...
  const urlPassword = ['p4ss', 'w0rd'].join('');
  const urlCredential = ['redis', '://', 'default', ':', urlPassword, '@', 'cache.internal:6379/0'].join('');
  const url = redactSecretLikeContent(`connect via ${urlCredential}`);
  assert.ok(!url.includes(urlPassword), 'URL-embedded credential must not survive');
...
test('URL-embedded credentials are also caught by the hard-reject detector', () => {
  const urlCredential = ['postgres', '://', 'user', ':', 's3cr3tpw', '@', 'db:5432/app'].join('');
  assert.equal(containsSecretLikeContent(urlCredential), true);
});
...
test('post-redaction invariant: redactor output is clean per the detector (redactor ⊇ detector)', () => {
  const dirty = `${['token:', ' eyJabc123.def456ghi.jkl789mno'].join('')} and ${['redis://u:', 'p@h'].join('')} and the secret is topsecretvalue`;
