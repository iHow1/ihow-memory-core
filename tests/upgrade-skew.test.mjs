...
test('rollback-runtime previews by default and atomically swaps only a self-verifying previous bundle with --apply', async (t) => {
  ...
  assert.deepEqual({ ok: preview.ok, applied: preview.applied, from: preview.from, to: preview.to }, { ok: true, applied: false, from: '0.1.0-alpha.33', to: '0.0.0-previous' });
  ...
  assert.deepEqual({ ok: applied.ok, applied: applied.applied, from: applied.from, to: applied.to }, { ok: true, applied: true, from: '0.1.0-alpha.33', to: '0.0.0-previous' });
  assert.equal(JSON.parse(await fs.readFile(path.join(runtime, 'package.json'), 'utf8')).version, '0.0.0-previous');
  assert.equal(JSON.parse(await fs.readFile(path.join(previous, 'package.json'), 'utf8')).version, '0.1.0-alpha.33', 'the displaced current generation becomes the next recovery source');
