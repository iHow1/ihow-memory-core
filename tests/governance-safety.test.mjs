...
const fakeAssignedApiKey = ['api_', 'key', ': ', 'ABCDEF', '0123456789'].join('');
...
const SECRET_FIXTURES = {
  'assignment api_key': fakeAssignedApiKey,
...
test('redactSecretLikeContent is same-source: removes the value AND clears the hard detector', () => {
  ...
  assert.doesNotMatch(redactSecretLikeContent(fakeAssignedApiKey), /ABCDEF0123456789/);
...
test('redactIngestBenign redacts email but leaves real secrets for the hard-reject gate', () => {
  ...
  const secret = redactIngestBenign(`rotate ${fakeAssignedApiKey} tonight`);
...
test('redactIngestBenign NEVER masks a real secret via the email overlap (URL creds / mixed)', () => {
  const serviceUri = (scheme, user, host, suffix) => [scheme, '://', user, ':', '***', '@', host, suffix].join('');
  const urlCreds = [
    `connect via ${serviceUri('redis', 'default', 'cache.internal:6379', '/0')}`,
    serviceUri('postgres', 'user', 'db.example.com:5432', '/app'),
    serviceUri('mongodb', 'admin', 'cluster0.mongodb.net', '/test'),
  ];
  ...
  const mixed = `rotate ${fakeAssignedApiKey} then notify bob@example.com`;
