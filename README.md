...
Alpha.32 prerelease candidate `0.1.0-alpha.32` (local release-ready only — the npm badge above shows the latest published version; see [CHANGELOG.md](./CHANGELOG.md)). Maturity is **alpha + single-machine real-app smoke**: Claude Code is dogfooded daily and has the richest native-hook path; Codex has native SessionStart / PreCompact / UserPromptSubmit hooks plus a proactive AGENTS memory loop; OMP now has a managed lifecycle extension and readable local transcripts; Hermes has packaged lifecycle and compaction adapters; the other runtimes have the narrower evidence recorded in [Runtime support](#runtime-support). Node >= 22.12 is a hard requirement (`node:sqlite`). Validated on macOS and Linux; native Windows is **experimental** and WSL is the supported path. The npm package includes the compiled CLI, stdio MCP server, read-only local console, OMP lifecycle extension, packaged Hermes adapters, privacy contract, and evidence-first release assets. Alpha releases may make breaking changes.

**Alpha.32 engineering detail:** bounds Core, CLI, console, and MCP reads to an 8,000-character preview unless full content is explicitly requested, and adds OMP session-start/prompt recall, native PreCompact checkpoints, exact session switch/shutdown capture, transcript discovery, managed wiring verification, activation evidence, and runtime capabilities. Replayable managed lifecycle completion remains `ACTIVATION_COMPLETION_UNATTESTED` and cannot claim `ACTIVE`. npm `@next` remains the source of truth for package availability; publication does not itself update a frozen runtime or authenticate a host.
