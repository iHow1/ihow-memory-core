...
**Alpha.31 local candidate (`0.1.0-alpha.31`; local release-ready only):** carries forward the complete Alpha.30 evaluation, proposal-review, temporal-recall, and gate-stability milestone, then adds deterministic turn receipts/replay, ordinary-language governed capture, checkpoint evidence prerequisites, deeper Hermes lifecycle integration, and measured `bge-m3` / 1024-dimension Metal semantic activation. The candidate passed its local joint engineering and real-provider gates, but has not been published, pushed, tagged, deployed, or activated by a runtime restart. Safe Memory Gardener remains review-first; continuous consolidation is report-only and never automatically rewrites authoritative memory.
