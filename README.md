...
**Alpha.31 prerelease (`0.1.0-alpha.31`; local release-ready):** carries forward the complete Alpha.30 evaluation, proposal-review, temporal-recall, and gate-stability milestone, then adds deterministic turn receipts/replay, ordinary-language governed capture, checkpoint evidence prerequisites, deeper Hermes lifecycle integration, and measured `bge-m3` / 1024-dimension Metal semantic activation. It also packages three isolated, non-live primitives: an idempotent report-only organize tick plus proposal review states (with no `APPLIED` or authoritative-write path), Grounded Media metadata evidence that reports only `EQUAL_UNTRUSTED`, and a bounded Activity Ledger where `COMMITTED` does not imply task success. The candidate passed its local joint engineering, real-provider, focused integration, and independent review gates. npm `@next` is the source of truth for package availability; publication does not imply runtime activation or production certification. Safe Memory Gardener remains review-first; continuous consolidation is report-only and never automatically rewrites authoritative memory.
