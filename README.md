...
Want to inspect before running? Read the [30–45 second evidence storyboard](./docs/demo-storyboard.md) or run the [repo-local synthetic demo](./examples/verify-first-handoff-demo.sh).
...
| DeepSeek Harness (DSH) | Separate [`dsh-ihow-memory`](https://github.com/iHow1/dsh-ihow-memory) package | checkpoint/MCP | official `0.1.1-rc.2` Host smoke: session-start handoff, first-step recall, after-turn observation, native compaction, and partial session-end checkpoint; adapter pins Core exactly |
...
Runnable, self-contained walkthroughs live in [`examples/`](./examples/), including the short [verify-first handoff demo](./examples/verify-first-handoff-demo.sh). All examples use synthetic data only.
...
Alpha.33 prerelease candidate `0.1.0-alpha.33` (local release-ready only — the npm badge above shows the latest published version; see [CHANGELOG.md](./CHANGELOG.md)). Maturity is **alpha + single-machine real-app smoke**: Claude Code is dogfooded daily and has the richest native-hook path; Codex has native SessionStart / PreCompact / UserPromptSubmit hooks plus a proactive AGENTS memory loop; OMP has a managed lifecycle extension and readable local transcripts; Hermes has packaged lifecycle and compaction adapters; the separately published DSH adapter has one-machine Host smoke against official `0.1.1-rc.2`; the other runtimes have the narrower evidence recorded in [Runtime support](#runtime-support). Node >= 22.12 is a hard requirement (`node:sqlite`). Validated on macOS and Linux; native Windows is **experimental** and WSL is the supported path. The npm package includes the compiled CLI, stdio MCP server, read-only local console, OMP lifecycle extension, packaged Hermes adapters, the DSH Core contract, privacy contract, and evidence-first release assets. Alpha releases may contain breaking changes.

**Alpha.33 engineering detail:** adds a bounded DSH Host adapter API and capability evidence for session-start handoff injection, first-step prompt recall, after-turn observation, native compaction checkpoints, and partial session-end checkpoints. Configuration, session, prompt, and checkpoint inputs stay hashed or omitted from activation evidence. Replayable managed lifecycle completion remains `ACTIVATION_COMPLETION_UNATTESTED` and cannot claim `ACTIVE`; Core publication alone does not install, update, or activate `dsh-ihow-memory`. npm `@next` remains the source of truth for package availability.
