...
**Alpha.27.1 local candidate (`0.1.0-alpha.27.1`; local release-ready only):** Alpha.27's bounded immutable Checkpoint Core, native Claude Code/Codex `PreCompact`, crash-floor, checkpoint-first continue, protection state, and same-HEAD `statusHash` / verify-first Git hardening remain unchanged. This patch makes an explicit managed `root` override ambient memory-root variables, sanitizes ambient routing variables from test child processes with subprocess proof, and separates success-path harness timing from the production 8 s watchdog / 10 s host envelope. The package still carries the Hermes Plugin and `ihow-memory-hermes-bridge` for the Hermes native lifecycle; prior adapter-lane evidence supports only `HOST VERIFIED/READY`, and the merged package is not independently certifiable as `ACTIVE`. Alpha.27 was published on npm; this Alpha.27.1 checkout has not been published and has not yet been pushed, tagged, released, or deployed, and it is not a production certification.
