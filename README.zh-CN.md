...
> AI agent 的本地优先共享记忆——零依赖 FTS5 检索、引用（citation）、治理流、stdio MCP。
...
iHow Memory 让你的多个 AI agent 在你自己的机器上共用一份持久记忆。记忆本体是磁盘上的纯 Markdown；检索用 Node 内置的 SQLite FTS5，每条结果都带引用；写入走显式治理流，而不是静默自写。Agent 通过 stdio MCP server 接入；你在 CLI 里用的是同一套流程。

## 为什么不同

1. **本地优先。** 完全在你的机器上运行：无账号、默认无遥测、无必需的网络调用。记忆是人类可读的 Markdown，可编辑、可 diff、可回滚。
2. **受治理的写入。** Agent 只能提出 candidate；升级（promote）为持久记忆是一个显式、留审计的步骤，读取结果自带引用——没有任何 agent 能静默改写共享记忆。
3. **一份记忆，多个 runtime。** Claude Code、Codex、Cursor 各一条命令，接入同一份本地记忆。
...
```bash
npx ihow-memory connect --runtime claude-code   # 或：codex | cursor
```
...
我们公开一项 LongMemEval_S 检索阶段结果：recall_all@10 = 1.0，覆盖全部 470 条有效样本（原始 500 条；ndcg_any@10 为 0.946）。三条边界：（1）这是检索层召回率，不是端到端、由 LLM 评判的答案准确率——不能与其他厂商报告的 90%+ 数字直接比较，后者度量的是另一层；（2）该成绩产生于我们实验性的「向量 + 词法」混合通道，而当前发布的包默认使用零依赖的 FTS5 词法检索（可选配本地向量 provider）；（3）一键复现工具链仍在开发中——在它落地之前，公开的 evidence manifest（指标定义、运行产物、完整的 @5 披露，含结构性上限）是可审计的依据。
...
ihow-memory connect          自动配置 runtime（claude-code | codex | cursor）[--dry-run]
...
- 规范与一致性（开放标准）：[iHow1/ihow-memory-standard](https://github.com/iHow1/ihow-memory-standard)
