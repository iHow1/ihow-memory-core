...
**Alpha.31 本地候选（`0.1.0-alpha.31`；local release-ready only）：** 完整继承 Alpha.30 的评测、提案审阅、时间召回与 Gate 稳定性能力，并新增确定性 turn receipt/replay、普通语言治理式捕获、checkpoint 证据前置条件、更深入的 Hermes 生命周期集成，以及真实 `bge-m3` / 1024 维 Metal 语义激活。该候选已通过本地联合工程与真实 provider Gate，但尚未发布，也尚未 push、tag、deploy 或通过 runtime 重启正式启用。Safe Memory Gardener 仍坚持 review-first；持续整理目前仅 report-only，绝不会自动改写权威记忆。
