...
```bash
npx ihow-memory@next connect --runtime claude-code   # 或：codex | cursor | workbuddy | claude-desktop | opencode | hermes | openclaw | vscode | gemini
```
...
| `next` | 增加**确定式 SessionStart floor** 兜底（单 cwd、低权重、离线验证过）+ **recall 开启**（🟢 reviewed-only、按相关性门控、带标签；🟡 auto 档 opt-in） |
...
- **自动捕获笔记是低权重、未经审阅**的，确定式 floor 是兜底、尚非 primary/默认权重路径。`recall` 默认只注入 🟢 **reviewed** 记忆——未经审阅的自动捕获 / journal 通道永不自动注入；机器判的 🟡 auto 档需 opt-in。可信长期记忆请用 `promote` / `durable-promote`。
