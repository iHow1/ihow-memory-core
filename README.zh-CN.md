...
**Alpha.31 预发布版（`0.1.0-alpha.31`；本地 release-ready）：** 完整继承 Alpha.30 的评测、提案审阅、时间召回与 Gate 稳定性能力，并新增确定性 turn receipt/replay、普通语言治理式捕获、checkpoint 证据前置条件、更深入的 Hermes 生命周期集成，以及真实 `bge-m3` / 1024 维 Metal 语义激活。该版本还打包了三个相互隔离、默认不接 live 的原语：带 receipt、TTL 与锁的 report-only organize tick 和提案审阅状态（没有 `APPLIED` 或权威写入路径）、只输出 `EQUAL_UNTRUSTED` 的 Grounded Media 元数据证据，以及 `COMMITTED` 不代表任务成功的有界 Activity Ledger。该候选已通过本地联合工程、真实 provider、focused 整合和独立审查 Gate。npm `@next` 是软件包可用性的真相源；发布不代表 runtime 已激活，也不代表生产认证。Safe Memory Gardener 仍坚持 review-first；持续整理目前仅 report-only，绝不会自动改写权威记忆。
