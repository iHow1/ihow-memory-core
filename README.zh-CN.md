...
**Alpha.31.1 预发布版（`0.1.0-alpha.31.1`；本地 release-ready）：** 包含完整 Alpha.31 能力集，并补上一组 runtime adapter 安全修复。WorkBuddy 用户级接入现在写入其实际生效的 `~/.workbuddy/.mcp.json`；Codex 只为缺失的只读接班工具补审批，保留用户更严格的策略，并在失败时事务级回滚注册与配置。冻结 `.runtime` 仍保持自包含、零外部运行依赖。这里证明的是文档所述 MCP 工具路径，并不宣称所有宿主生命周期都已 `ACTIVE`；在缺少生命周期证据时，`doctor` 仍可能报告 `TOOLS ONLY`、`READY — WAITING FOR FIRST ACTIVITY` 或 `NEEDS REPAIR`。npm `@next` 是软件包可用性的真相源；发布不代表 runtime 已激活，也不代表生产认证。Alpha.31 的 review-first 边界保持不变：持续整理仅 report-only，绝不会自动改写权威记忆；Grounded Media 只输出 `EQUAL_UNTRUSTED`；Activity Ledger 的 `COMMITTED` 不代表任务成功。
