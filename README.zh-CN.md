...
**Alpha.27 本地候选（`0.1.0-alpha.27`；local release-ready only）：** 有界、不可变的 Checkpoint Core 已接入 Claude Code 与 Codex 原生 `PreCompact`；陈旧但有效的 draft 有有界 partial checkpoint crash-floor，`memory.continue` 按 checkpoint-first 顺序优先使用 checkpoint，再退到 transcript 与 floor journal。`status` 会暴露 protection state，但不会把降级证据当作精确安全结论；现场验证会拒绝 same-HEAD `statusHash` drift，并在 checkpoint 缺失 `statusHash` 时 fail closed。git anchor 收集会禁用仓库可控的 `textconv` 与 `core.fsmonitor` 命令。Hook 在 watchdog 下仍对宿主 fail-open，checkpoint 持久化与信任判定则保持 fail-closed。npm 包也包含 Hermes Plugin 及其 `ihow-memory-hermes-bridge` 可执行 bridge，用于接入 Hermes 原生生命周期；此前 adapter lane 的真机证据只支持有界的 `HOST VERIFIED/READY` 口径，合并后的包不能独立认证为 `ACTIVE`。当前 checkout 仍是 local release-ready only，尚未发布、push、tag、release 或 deploy，也不代表 production certification。
