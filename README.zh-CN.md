...
**Alpha.27 Stage 3 候选状态（尚未发布）：** 有界 draft / 不可变 hash-addressed checkpoint core 已通过 `setup` / `install-hook` 接入 Claude Code 与 Codex 的原生 `PreCompact`。Adapter 不读取或持久化 transcript / custom instructions 原文，使用有界私有 draft/artifact 索引，保留第三方 Hook 配置，并以 2.5 秒 watchdog 对宿主 fail-open；持久化 checkpoint 的完成判定仍然 fail-closed。Checkpoint artifact **尚未接入 `memory.continue`**，尚无 checkpoint crash-floor 路径，也尚未 publish。
...
MCP 工具与治理闭环与 runtime 无关。Claude Code 使用 skill + Stop / SessionStart / PreCompact / UserPromptSubmit hooks；Codex 使用原生 SessionStart / PreCompact / UserPromptSubmit hooks，并由 `~/.codex/AGENTS.md` 提供主动记忆循环。Stage 3 的 PreCompact checkpoint adapter 有界、无 transcript 原文、对宿主 fail-open；它尚未接入 `memory.continue`，也不提供 checkpoint crash floor。Resume 提示会自动注入到配置暴露了指令文件的 runtime（Claude Code、WorkBuddy、OpenClaw、Hermes、OpenCode）。
